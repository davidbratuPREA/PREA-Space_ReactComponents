...
/** Scales a fixed 1920×1080 app frame to the width of its container (no chrome). */
export function ScaledScreen({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div ref={ref} className={className} style={{ position: 'relative', width: '100%', aspectRatio: '16 / 9', overflow: 'hidden' }}>
      <div className="demo-frame__inner" style={{ transform: `scale(${scale})` }}>{children}</div>
    </div>
  );
}
...
export function DemoFrame({ children }: { children: React.ReactNode }) {
  return <ScaledScreen className="demo-frame">{children}</ScaledScreen>;
}
...
/** Collapsed MainNav rail + BottomNav around the screen content (Figma app frame). */
export function AppShell({ active = 'chat', children, bottomLeft }: AppShellProps) {
  return (
    <div className="app">
      ...
      <div className="app__bottom">
        <BottomNav icons={['li:fingerprint-pattern', 'li:fingerprint-pattern', 'li:fingerprint-pattern', 'li:fingerprint-pattern']} date="Aug 01" time="09:43">
          {bottomLeft ?? (
            <>
              <BottomNavButton icon="li:colors">Identity</BottomNavButton>
              <BottomNavButton icon="li:users">People</BottomNavButton>
              <BottomNavButton icon="li:layers">Build</BottomNavButton>
              <BottomNavButton icon="li:euro">Capital</BottomNavButton>
              <BottomNavButton icon="li:map" variant="grey">Deep Street</BottomNavButton>
              <BottomNavButton icon="li:sliders">Control</BottomNavButton>
            </>
          )}
        </BottomNav>
      </div>
